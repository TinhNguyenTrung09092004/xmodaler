...
This distribution has builtin support for the MSCOCO image captioning dataset used by COS-Net. The corresponding dataset wrappers are provided in `./xmodaler/datasets`:
```
xmodaler/datasets/
  images/
    mscoco.py
    mscoco_cosnet.py
```
You can specify which dataset wrapper to use by `DATASETS.TRAIN`, `DATASETS.VAL` and `DATASETS.TEST` in the config file. COS-Net uses `MSCoCoCOSNetDataset`.

# Expected structure for xmodaler
First, download the [dataset files](https://drive.google.com/drive/folders/1vx9n7tAIt8su0y_3tsPJGvMPBMm8JLCZ?usp=sharing) and [coco_caption](https://github.com/ruotianluo/coco-caption), plus the COS-Net specific files listed in [configs/image_caption/cosnet/README.md](../../configs/image_caption/cosnet/README.md).
...
mscoco_dataset/
  ...
  # the COS-Net semantic files (see configs/image_caption/cosnet/README.md)
features/
  CLIP_RN101_49/
      *.npz
```
